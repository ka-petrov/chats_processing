"""
To use VK API you'll need a VK account and an access_token. Follow the docs
page: `https://vk.com/dev/implicit_flow_user` for the complete instructions.

My simplified instructions are as follows:
1) create a VK application: `https://vk.com/editapp?act=create`
2) find it here: `https://vk.com/apps?act=manage`, go to the app settings and
   copy the app ID
3) paste this link into your browser and replace `YOUR_APP_ID` with the ID
...
4) you'll be redirected and your new URL will contain the `access_token`
   field; copy it, paste to `settings.vk_token` and you're ready to go!
5) note that `access_token` has an expiry, so after it expires you'll have to
   repeat steps 3-4
"""
...
def api_call(request, **kwargs):
    ...
    while not ok:
        try:
            ...
            # if we hit the API requests limit (error code = 6), wait for some
            # time and retry
...
if __name__ == '__main__':
    ...
    for i in range(0, n, max_items):
        ...
        # the inner loop iterates through all dialogs and saves user and chat
    ...
    for i, u in enumerate(users):
        ...
        for i in range(0, n, max_items):
            time.sleep(api_delay)
            print('%d / %d' % (i, n))
            messages = api_call(vk_api.messages.getHistory, peer_id=u,
